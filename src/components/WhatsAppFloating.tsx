    ...
    <div className="fixed bottom-6 right-6 z-50 select-none pointer-events-none">
      ...
      >
        <AnimatePresence>
          {showTooltip && (
            <motion.div
              ...
              className="absolute bottom-16 left-1/2 px-3.5 py-2 rounded-2xl bg-on-surface/95 text-on-primary font-sans text-[11px] font-bold uppercase tracking-wider shadow-xl border border-white/10 whitespace-nowrap z-[60] pointer-events-none"
            >
        ...
        <motion.a
          ...
          whileTap={{ scale: 0.92 }}
          className="w-full h-full rounded-2xl bg-gradient-to-tr from-[#25D366] to-[#128C7E] text-on-primary flex items-center justify-center shadow-xl border border-white/20 relative overflow-hidden block"
          onMouseEnter={() => !isDraggingRef.current && setShowTooltip(true)}
          onMouseLeave={() => setShowTooltip(false)}
        >
          {/* Internal subtle glow reflection */}
          <div className="absolute inset-0 bg-surface/10 opacity-0 hover:opacity-100 transition-opacity" />
          
          {/* WhatsApp Official Logo SVG */}
          <svg className="w-7 h-7 fill-current text-on-primary" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
